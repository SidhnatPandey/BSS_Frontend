...
import { faDashboard, faDatabase, faEye, faGlobe, faHome, faIdCard, faSignOut, faStore, faUsersLine } from '@fortawesome/free-solid-svg-icons';
...
export class SidebarComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    const a = this.admin_data+","+this.my_acct+","+this.ai+","+this.master_database
    const b= a.split(",")
    console.log(b)
  ...
  logoutUser() {
    ...
    if (this.user_name != 'SUPER ADMIN') {
      this.http.post("http://localhost:5000/removeStatus",body).subscribe(
        (response) => {
